...
const registerPlayerEvents = (player, client) => {
    ...
    player.events.on('audioTrackAdd', async (queue, track) => {
        if (queue.isPlaying()) {
            ...
            await queue.metadata.channel.send({ embeds: [embed.Embed_add('追加された音楽', track.title, track.url, track.thumbnail, author, trackDuration)] });
            ...
            queue.dashboard = await queue.metadata.channel.send({ embeds: [embed.Embed_dashboard('音楽の情報', cur.title, cur.url, cur.thumbnail, settings(queue))], components: [row] });
            return;
        }
    });
