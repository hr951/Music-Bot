...
module.exports = {
    ...
    aliases: ['h'],
    showHelp: false,
    description: 'コマンドの使い方を確認する',
    usage: 'help [command]',
    options: [
        {
            name: "command",
            description: "ヘルプが必要なコマンドを入力",
            type: 3,
            required: false
        }
    ],
    ...
    execute(client, message, args) {
        ...
        if (!args[0]) {
            ...
            if (!Boolean(found)) return message.reply({ content: '❌ | コマンドが見つかりません', allowedMentions: { repliedUser: false } });
        }
    },
    ...
    slashExecute(client, interaction) {
        ...
        if (!command) {
            ...
            if (!Boolean(found)) return interaction.reply({ content: '❌ | コマンドが見つかりません', allowedMentions: { repliedUser: false } });
        }
    },
};
